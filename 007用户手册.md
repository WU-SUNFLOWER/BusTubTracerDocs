...
作为一个计算机软件，BusTub在处理数据表时，必须要将数据表加载到内存中进行处理。作为一名数据库原理的学习者，了解BusTub内部是如何在内存中存储和管理数据表，是十分必要的。

在介绍这个问题之前，我首先需要向你简单介绍一下BusTub中内存页（Page）的概念。一个内存页就是一块大小为4KB的内存空间。在BusTub启动时，首先会向操作系统申请若干份这样的内存空间，即申请若干张内存页。内存页在BusTub中主要有两种用途，其一是用于存储数据表的数据，其二是用于存储数据表的索引信息。这里我将重点介绍前者，而后者将会放在后续章节中介绍。

> 4KB的大小对于可视化演示来说，显然太大了。因此在BusTubTracer的实现中，我们将一个内存页的大小限定为134B。作为弥补，在数据库系统启动时会向操作系统申请更多数量的内存页。

BusTub会通过内存池（Buffer Pool）来管理内存页的申请和释放。内存池中有一个所谓的空闲列表（Free List）的数据结构，用于管理所有的空闲的（即待分配的）内存页。当BusTub启动时，其向操作系统申请到的内存页都会被挂到空闲列表上，之后随着程序的推移，空闲列表上的内存页可能会被分配出去，用于存储数据表数据或者索引，也有可能随着某张数据表被删除等操作而随之被释放，重新被数据库系统回收到空闲列表当中。如果BusTub空闲列表中所有的内存页都已经被耗尽，而这时候数据库系统又想要申请更多的内存页空间，那么BusTub会采用一定的页替换算法（在BusTubTracer的实现中为LRU-K），将某个不常用的已分配内存页中的数据转储到硬盘上。那么这时当前内存页对应的这块内存空间就又空出来了，可以再次被分配出来以满足当务之急。如果你曾经学习过操作系统虚拟内存管理方面的知识，相信这对你来说非常熟悉。
